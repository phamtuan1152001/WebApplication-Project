...
function AddBestSellers() {
    ...
    const handleAdd = async (e) => {
        ...
        try {
            ...
            let res = await fetch(`http://localhost:5000/admin/add-bestseller/${idAdd}`, {
              method: "POST",
              headers: {
                "Authorization": `${user.token}`
              }
                ...
                {bestSellers.map((bestSeller) => (
                  <div className="card col-4" key={bestSeller._id}>
                    ...
                    <img
                      ...
                      alt={bestSeller.Name}
                    />
                      <h5 className="card-title">{bestSeller.Name}</h5>
                      <p className="card-text">
                        Designs:{" "}
                        {bestSeller.Descriptions &&
                          bestSeller.Descriptions.Designs}
                      </p>
                      <p className="card-text">
                      Material:{" "}
                        {bestSeller.Descriptions &&
                          bestSeller.Descriptions.Material}
                      </p>
                      <p className="card-text">
                      Origin:{" "}
                        {bestSeller.Descriptions &&
                          bestSeller.Descriptions.Origin}
                      </p>
                        <input 
                            type="radio" 
                            onChange={() => setIdAdd(bestSeller._id)}
                        />
                    </div>
                  </div>
                ))}
